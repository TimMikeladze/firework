...
/**
 * Which voice the break fires. `auto` lets the break's own spread pick one;
 * the rest name a character directly. Must match `BURST_CHARACTERS` in
 * `sfx.ts` — named here rather than imported so the data model keeps no
 * dependency on the audio graph.
 */
export type BurstVoice = "auto" | "salute" | "shell" | "deep" | "bomb";

export const BURST_VOICES: readonly BurstVoice[] = [
  "auto",
  "salute",
  "shell",
  "deep",
...
export function defaultShell(overrides: ShellOverrides = {}): ShellSpec {
  ...
  return {
    ...
    audio: { enabled: true, boom: 0.7, crackle: 0.5, voice: "auto" },
    ...rest,
  };
}
...
export function parseShell(input: unknown): ShellSpec {
  ...
  return {
    ...
    audio: {
      ...
      voice: BURST_VOICES.includes(audio.voice as BurstVoice)
        ? (audio.voice as BurstVoice)
        : base.audio.voice,
    },
  };
}
