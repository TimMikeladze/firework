...
export const PRESETS: readonly ShellSpec[] = [
  preset("preset-vercel-triangle", "Vercel Triangle", {
    launch: {
      height: 38,
      // Dead vertical: the mark is aimed along the shell axis, so any tilt
      // leans the triangle over.
      tilt: 0,
      fuse: 0.05,
      trailRate: 55,
      trailColor: "#dfe9ff",
    ...
    // Low turbulence and a light gravity share keep the three edges straight
    // for as long as the sparks burn.
    physics: { gravity: 6.8, drag: 0.3, wind: 0.2, turbulence: 0.1 },
    layers: [
      defaultLayer({
        name: "Mark",
        pattern: "triangle",
        count: 3200,
        speed: 18,
        // Almost no jitter: the outline is the whole point.
        speedJitter: 0.03,
        life: 2.6,
        lifeJitter: 0.12,
        size: 0.1,
      ...
      defaultLayer({
        name: "Gradient",
        pattern: "triangle",
        // Wide enough to clear the white edge's own bloom, and a beat later,
        // so the blue reads as a fringe on the mark rather than a second
        // triangle behind it.
        count: 1800,
        speed: 21,
        speedJitter: 0.03,
    ...
    // A detonation rather than a report: the mark should land like something
    // went off, not like a shell opened.
    audio: { enabled: true, boom: 0.9, crackle: 0.3, voice: "bomb" },
    // Glassy water: the mirror pass hands back a second, upside-down mark.
    look: {
      bloom: 1.2,
      exposure: 1.05,
      reflection: 0.75,
