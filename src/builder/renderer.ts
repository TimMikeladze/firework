...
const WATER_Y = 0;
/** Lowest the eye may sit above the water plane. */
const MIN_EYE_Y = WATER_Y + 1.5;
...
async function bootFireworks(
  ...
  function launchShell(
    ...
    if (design.audio.enabled && !muted) {
      audio.resume();
      const { distance, pan } = listenerSpace(rocket.x, rocket.y, rocket.z);
      audio.lift(design.audio.boom * 0.6, rocket.fuse, distance, pan);
    }
  }

  /**
   * Where a point in the scene sits relative to the listener: how far, and how
   * far across. The camera is the listener, so a break behind the eye pans the
   * way the eye would hear it, and the pan comes from the camera's own right
   * vector rather than from world x — which would be wrong the moment the show
   * is orbited.
   */
  function listenerSpace(
    x: number,
    y: number,
    z: number,
  ): { distance: number; pan: number } {
    const dx = x - cameraUniform.eye[0];
    const dy = y - cameraUniform.eye[1];
    const dz = z - cameraUniform.eye[2];
    const distance = Math.hypot(dx, dy, dz);
    if (distance < 0.001) return { distance: 0, pan: 0 };
    const right = cameraUniform.right;
    const across = (dx * right[0] + dy * right[1] + dz * right[2]) / distance;
    // Softened: a hard-panned break sounds like it left the show.
    return { distance, pan: Math.max(-1, Math.min(1, across)) * 0.75 };
  }
  ...
  function breakShell(rocket: Rocket) {
    ...
    if (design.audio.enabled && !muted) {
      ...
      // `auto` is the absence of a choice, not a character: leaving it off
      // lets the break's own spread pick one.
      audio.boom(
        design.audio.boom,
        spread,
        distance,
        pan,
        design.audio.voice === "auto" ? undefined : design.audio.voice,
      );
      const tail = firstLayer ? Math.min(3, firstLayer.life) : 1;
      audio.crackle(design.audio.crackle, tail, distance, pan);
    }
  }
  ...
  /**
   * The orbit's pitch floor is an angle, so far enough out it still swings the
   * eye under the water — where the sky pass has no surface in front of it and
   * the show hangs upside down. Re-clamp against the eye's real height, which
   * is what actually has to stay dry.
   */
  function keepAboveWater() {
    const sin =
      (MIN_EYE_Y - controls.target[1]) / Math.max(1e-3, controls.distance);
    if (sin <= -1) return;
    const floor = Math.asin(Math.min(1, sin));
    if (controls.pitch < floor) controls.set({ pitch: floor });
  }
