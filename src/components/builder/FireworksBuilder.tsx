...
const MOBILE_TABS = [
  { id: "shell", label: "Design" },
  { id: "rack", label: "Rack" },
  { id: "music", label: "Music" },
] as const satisfies readonly { id: MobileTab; label: string }[];

/** Trade names for the break voices. "Auto" reads the break's own spread. */
const VOICE_LABELS: Record<BurstVoice, string> = {
  auto: "Auto",
  salute: "Salute",
  shell: "Shell",
  deep: "Deep",
...
export default function FireworksBuilder() {
  ...
  const exportJson = useCallback(() => {
    ...
    void navigator.clipboard
      ?.writeText(json)
      .then(() => setStatus("Firework JSON copied to the clipboard"))
      .catch(() => setStatus("Could not reach the clipboard"));
  }, [spec]);

  const importJson = useCallback(() => {
    const text = window.prompt("Paste firework JSON");
    if (!text) return;
    try {
  ...
  const shellCard = (
    <div className="flex flex-col gap-4">
        ...
        <DeskButton
          ...
          title={
            spec.layers.length >= MAX_LAYERS
              ? `A firework carries at most ${MAX_LAYERS} layers`
              : undefined
          }
        >
          Add layer
      ...
      <Section title="Sound">
        ...
        <Segmented
          label="Voice"
          value={spec.audio.voice}
          options={BURST_VOICES}
          format={(voice) => VOICE_LABELS[voice]}
          onChange={(voice) => patch({ audio: { ...spec.audio, voice } })}
        />
      </Section>
    </div>
  );
  ...
  const autoFireControl = (
    <label className="flex min-w-0 flex-1 items-center gap-2 text-[11px] lg:flex-none">
      ...
      <input
        ...
        step={1}
        value={autoFire}
        aria-label="Auto-fire rate, fireworks per minute"
  ...
  const deskButtons = (
    <>
      <DeskButton onClick={randomize} title="Roll a random firework (R)">
        Roll
      </DeskButton>
      <DeskButton
        onClick={() => {
