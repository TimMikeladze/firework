    ...
    const tick = () => {
      ...
      if (liveReadoutRef.current) {
        liveReadoutRef.current.textContent = state.locked
          ? `${Math.round(state.bpm)} bpm · ${state.shells} fired`
      ...
      {open && track ? (
        <>
          ...
          <svg
            ...
            role="img"
            aria-label={`Show timeline: ${cueCount} cues across ${formatTime(duration)}`}
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            <span className="readout text-ash text-[10px]">
              {plan ? `${Math.round(plan.bpm)} bpm` : "—"} · {cueCount} cues ·{" "}
              {summary
        ...
        <>
          ...
          <p className="text-ash mt-1.5 text-[10px] leading-relaxed">
            {listening === "tab"
              ...
              : "Every firework is aimed at a beat that has not happened yet — a steady tempo gives the tightest sync."}
          </p>
          {settings}
        </>
      ) : null}
